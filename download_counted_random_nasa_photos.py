...
def create_parser():
    ...
    parser.add_argument("-n", "--number_of_downloaded_photos", nargs="?",
                        default=5)
    return parser
...
def get_nasa_download_urls(apod_url, api_key, photos_count):
    ...
    return [element["url"] for element in response_elements]
...
def main():
    ...
    download_pictures(
        get_nasa_download_urls(
            NASA_URL_APOD,
            nasa_api_key,
            number_of_download_photos),
        download_path,
        "nasa_apod",
    )
